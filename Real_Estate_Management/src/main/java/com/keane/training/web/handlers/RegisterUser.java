...
public class RegisterUser implements HttpRequestHandler 
{
	...
	public void handle(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		boolean isExists;
		try {
			isExists = dao.validateSeller(user.getEmail()); // is this user already existed ? / not 
			...
			} else 
			{
				...
				user.setPassword("seller@"+ Integer.toString(user.getSid()));
				int finalRes = dao.registerSeller(user);
